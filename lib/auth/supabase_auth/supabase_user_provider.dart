...
class BekriSupabaseUser extends BaseAuthUser {
  BekriSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> bekriSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BekriSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
